...
export default function CardActions({ data }) {
  ...
  async function updateActivation(prev) {
    const response = await apiFetch(
      `http://localhost:4000/users/clients/${data._id}`,
      {
        method: "PUT",
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[180px]">
          ...
          <DropdownMenuItem
            className="flex items-center gap-2"
            onSelect={() => updateActivation(data.isActive)}
          >
            {!data.isActive ? (
              <>
      ...
      <DeleteModal
        ...
        route="http://localhost:4000/users/clients"
